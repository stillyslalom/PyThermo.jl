...
`Pc`, `omega`, `Tb`, `Hvap`, `Psat`) are **optional**: their return type is
`Union{Quantity, Missing}`. They return `missing` when thermo returns
`None`.
...
| `Z` / `Zg` / `Zl` / `Zs`                  | `compressibility(c[, phase])`      | —          |
...
internals are unexported but stable enough that external packages can use
them to define additional wrappers without duplicating the boilerplate.
